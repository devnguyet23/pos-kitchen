...
let UsersService = class UsersService {
    ...
    async create(createUserDto, currentUser) {
        const { username, email, password, chainId, storeId, roleId } = createUserDto, rest = __rest(createUserDto, ["username", "email", "password", "chainId", "storeId", "roleId"]);
        ...
        const user = await this.prisma.user.create({
            data: Object.assign({ username,
                email, password: hashedPassword, chainId: chainId || currentUser.chainId, storeId: storeId || currentUser.storeId }, rest),
    ...
    async findAll(params, currentUser) {
        ...
        const [data, total] = await Promise.all([
            this.prisma.user.findMany({
                ...
                select: Object.assign(Object.assign({}, this.getUserSelect()), { userRoles: {
                        where: { isActive: true },
    ...
    async findOne(id, currentUser) {
        const user = await this.prisma.user.findUnique({
            where: { id },
            select: Object.assign(Object.assign({}, this.getUserSelect()), { chain: { select: { id: true, name: true, code: true } }, store: { select: { id: true, name: true, code: true } }, userRoles: {
                    where: { isActive: true },
                    include: {
                        role: {
                            include: {
    ...
    async update(id, updateUserDto, currentUser) {
        ...
        const { chainId, storeId, roleId } = updateUserDto, rest = __rest(updateUserDto, ["chainId", "storeId", "roleId"]);
        const updatedUser = await this.prisma.user.update({
            where: { id },
            data: Object.assign(Object.assign(Object.assign({}, rest), (chainId !== undefined && {
